    ...
    for (let x = x_min; x <= x_max; x++)
        for (let y = y_min; y <= y_max; y++)
            for (let z = z_min; z <= z_max; z++){
                let count = countActiveNeghbours(active,x,y,z);
                //console.log(count);
                if ((active.has(`${x},${y},${z}`) && (count === 2 || count === 3)) ||
                    (!active.has(`${x},${y},${z}`) && (count === 3))){
                    //console.log("New active at",x,y,z);
                    temporaryActive.add(`${x},${y},${z}`);                   
...
function countActiveNeghbours(cube, x,y,z){
    let c = 0;
